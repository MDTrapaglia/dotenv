...
# Convenience alias to send all HTMLs in a path (file or folder) via configured transport
# alias sendhtml="python ~/projects/send_email/send_html_batch.py"

# Start ssh-agent only when explicitly enabled to avoid passphrase prompts on source
if [[ ${ENABLE_SSH_AGENT:-0} -eq 1 && $- == *i* && -t 1 ]]; then
